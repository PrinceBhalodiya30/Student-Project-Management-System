...
import { AlertCircle, ArrowRight, Bell, Calendar, CheckSquare, FileText, Send, User, Upload } from "lucide-react"
import Link from "next/link"

// Simple UploadDialog component since it's used in the JSX
function UploadDialog({ userId, projectId }: { userId: string, projectId: string }) {
    return (
        <Dialog>
            <DialogTrigger asChild>
                <Button variant="outline" size="sm" className="bg-cyan-500/10 border-cyan-500/20 hover:bg-cyan-500/20 text-cyan-400">
            ...
            <DialogContent className="glass-modern border-cyan-500/20">
                ...
                <FormWithToast
                    action={async (formData) => {
                        "use server"
                        return await uploadProjectFile(userId, projectId, formData)
                    }}
                ...
                >
                    <div className="space-y-2">
                        ...
                        <Input id="file" name="file" type="file" required className="bg-white/5 border-cyan-500/20" />
                    </div>
...
export default async function StudentProjectPage() {
                ...
                include: {
                    Project: {
                        include: {
                            ...
                            Milestone: true,
                            Announcement: { orderBy: { createdAt: 'desc' } }
                        }
                    },
    ...
    if (!student?.ProjectGroup) {
        return (
            <div className="flex flex-col min-h-screen w-full">
                <TopBar title="Project Details" user={{ name: student?.User.fullName, email: student?.User.email }} />
                <div className="p-6 space-y-6 animate-fade-in text-foreground relative z-10 w-full min-h-full">
    ...
    if (!project) {
        if (!student.isLeader) {
            return (
                <div className="flex flex-col min-h-screen w-full">
                    <TopBar title="Project Proposal" user={{ name: student.User.fullName, email: student.User.email }} />
                    <div className="p-6 space-y-6 animate-fade-in text-foreground relative z-10 w-full min-h-full">
                        <h1 className="text-3xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-cyan-400 to-blue-500">
                            Project Proposal
                        </h1>
        ...
        return (
            <div className="flex flex-col min-h-screen w-full">
                <TopBar title="Submit Proposal" user={{ name: student.User.fullName, email: student.User.email }} />
                <div className="p-6 space-y-6 animate-fade-in text-foreground relative z-10 w-full min-h-full">
                    <h1 className="text-3xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-cyan-400 to-blue-500">
                            ...
                            <TabsContent value="announcements" className="mt-4">
                                <Card className="glass-modern border-cyan-500/20">
                                    <CardHeader>
                                        ...
                                        <CardDescription>Stay updated with the latest instructions from your guide.</CardDescription>
                                    </CardHeader>
                                    <CardContent className="space-y-4">
                                        {project.Announcement.length === 0 ? (
                                            <div className="text-center py-12">
                                                <div className="inline-flex items-center justify-center p-4 rounded-full bg-cyan-500/10 mb-4">
                                                    <Bell className="h-8 w-8 text-cyan-500/50" />
                                                </div>
                                                <p className="text-muted-foreground text-sm">No announcements from your guide yet.</p>
                                            </div>
                                        ) : (
                                            project.Announcement.map(announcement => (
                                                <div key={announcement.id} className="p-5 rounded-2xl bg-white/5 border border-cyan-500/10 hover:bg-white/10 transition-colors">
                                                    <div className="flex justify-between items-start mb-3">
                                                        <div>
                                                            <h4 className="font-bold text-lg text-cyan-400">{announcement.title}</h4>
                                                            <p className="text-xs text-muted-foreground">{format(new Date(announcement.createdAt), "PPP p")}</p>
                                                        </div>
                                                        <Badge variant="secondary" className="bg-cyan-500/10 text-cyan-400 border-none">Official</Badge>
                                                    </div>
                                                    <div className="prose prose-invert prose-sm max-w-none">
                                                        <p className="text-foreground/90 whitespace-pre-wrap leading-relaxed">{announcement.content}</p>
                                                    </div>
                                                </div>
                                            ))
                                        )}
